...
class MenuController extends CommonController
{
    ...
    {
        $listorder = $_POST['listorder']; // array('menuId'=>menuValue,...)
        ...
        if ($listorder) {
            try {
                ...
                return show(0, $e->getMessage(), array(
                    'jump_url' => $jumpUrl
                ));
            }
            
            if ($errors) {
                return show(0, '排序失败-' . implode(',', $errors), array(
                    'jump_url' => $jumpUrl
                ));
            }
            return show(1, '排序成功', array(
                'jump_url' => $jumpUrl
            ));
        }
        return show(0, '排序数据失败', array(
            'jump_url' => $jumpUrl
        ));
    }
}
